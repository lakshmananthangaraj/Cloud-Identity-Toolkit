...
Function Get-GuestUsers
{
    ...
    # Define an empty array to hold all Guest users
    $GuestUsers = New-Object System.Collections.ArrayList
    $totalGuestUsers = 0

    # Define the initial URI to retrieve Guest users with a filter and select options
    $uri = "https://graph.microsoft.com/beta/users?`$top=100&`$filter=userType eq 'Guest'&`$select=id,userPrincipalName,mail,displayName,userType,accountEnabled,OnPremisesSyncEnabled,createdDateTime,department,signInActivity&`$count=true"
    ...
    do 
    {
        ...
        { 
            Try 
            {
                # Invoke the Graph API to retrieve Guest users
        ...
        if($partialData) 
        {
            $GuestUsersData = $partialData.content | ConvertFrom-Json
        }

        # Output the total number of Guest users retrieved so far
        Write-Host ""
        Write-Host "Progress: $($totalGuestUsers += $GuestUsersData.value.Count; $totalGuestUsers) Guest users retrieved so far" -ForegroundColor Cyan
        
        # Check if there are more pages of data to retrieve
        if ($GuestUsersData.PSObject.Properties['@odata.nextLink']) { $uri = $GuestUsersData.'@odata.nextLink' }

        # Flatten signInActivity safely
        $GuestUsersData.value | ForEach-Object {
    ...
    } until (-not($GuestUsersData.PSObject.Properties['@odata.nextLink']))

    # CSV EXPORT SUPPORT
    if($ExportFormat -eq "CSV" -and $ExportPath)
    {
        $GuestUsers | Export-Csv -Path $ExportPath -NoTypeInformation -Force

        Write-Host ""
        Write-Host "Guest users report exported successfully → $ExportPath" -ForegroundColor Green
    }

    # Return the array list containing all Guest users
    return $GuestUsers | Select-Object Id, displayName, userPrincipalName, userType, createdDateTime, accountEnabled, onPremisesSyncEnabled, lastSuccessfulSignInDateTime, lastSignInDateTime, lastNonInteractiveSignInDateTime | FT
}
