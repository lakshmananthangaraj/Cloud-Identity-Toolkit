...
.SYNOPSIS
    Retrieves PIM Eligible Entra ID Role Assignment Details and optionally generates an enhanced HTML dashboard report.

.DESCRIPTION
    This function retrieves all Privileged Identity Management (PIM) eligible role assignments
    from Microsoft Graph (beta endpoint) using the roleEligibilityScheduleInstances API.
    ...
    - Throttling (HTTP 429 retry handling)
    - Principal type resolution (User, Group, Service Principal)
    - Normalized output formatting for reporting and auditing
    - Optional HTML dashboard report generation with:
        ✅ Overview tab      — KPI cards, charts, security health score, layman-friendly explainers
        ✅ Users tab         — User-specific eligible role assignments with UPN, mail
        ✅ Groups tab        — Group-based eligible role assignments
        ✅ Service Principals tab — App/SP eligible role assignments
        ✅ Risk Insights tab — Permanent eligibility, high-privilege roles, stale access warnings
...
.PARAMETER AccessToken
    Specifies the Microsoft Graph access token used for authentication.

    This parameter is mandatory (when using this auth method) and must include
    permissions such as:
...
.PARAMETER TenantId
    The Azure AD Tenant ID (GUID). Serves double duty: it's used both to authenticate
    (when -ClientId/-ClientSecret are supplied) and to display in the dashboard header.
    If you already have an -AccessToken, this is only used for display.

.PARAMETER TenantName
    Display name of the tenant (e.g. "Contoso Ltd"). Shown in the dashboard header.

.PARAMETER GenerateHtmlDoc
    Switch parameter that generates an enhanced HTML dashboard report of eligible role assignments.

    When enabled, the report is saved to:
    C:\Temp\PIMEntraIDRoleEligibleAssignment_<TenantName>_<timestamp>.html

.EXAMPLE
    Get-PIMEligibleEntraIDRoleAssignmentDetails -AccessToken $AccessToken

    Retrieves all eligible Entra ID role assignments and returns PowerShell objects.

.EXAMPLE
    Get-PIMEligibleEntraIDRoleAssignmentDetails -AccessToken $AccessToken -TenantName "Contoso" -TenantId "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" -GenerateHtmlDoc

    Retrieves eligible role assignments and generates a full HTML dashboard report.

.EXAMPLE
    . .\Connect-EntraID.ps1
    $secret = Read-Host -Prompt "Client secret" -AsSecureString
    Get-PIMEligibleEntraIDRoleAssignmentDetails -ClientId "<app-id>" -ClientSecret $secret -TenantId "<tenant-id>" -GenerateHtmlDoc
...
.OUTPUTS
    ...
    Returns structured objects containing:
    - Assignment State
    - Assigned Type
    - Role Name
    - User Id
...
.NOTES
    ...
    types are limited to users only (no group/SP-based PIM eligibility), consider
    ...
    Behavior:
    - Supports both a manually-supplied AccessToken and app-only authentication via
      Connect-EntraID.ps1 (-ClientId/-ClientSecret/-TenantId)
    - Handles paging using @odata.nextLink
    - Safe null handling for missing properties
    - Designed for reporting and automation pipelines
    ...
    ─────────────────────────────────────────────────────────────────────────────
        ...
        2.0 (09-Jul-2026)      - Added app-only authentication support via
                               ...
                               - Replaced hardcoded C:\Temp output path with a new
                                 -OutputPath parameter (defaults to $env:TEMP).
                               - Fixed a throttle-retry loop bug where the retry
                                 condition always evaluated true after one attempt
                                 (was an accidental assignment instead of a
                                 comparison), so 429 responses now genuinely retry.
...
Function Show-FriendlyHelp
{
    ...
    Write-Host "  ║  Get-PIMEligibleEntraIDRoleAssignmentDetails — Friendly Help  ║" -ForegroundColor Cyan
    Write-Host "  ╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  What this does:" -ForegroundColor Yellow
    Write-Host "    Pulls every PIM-eligible role assignment from your Entra ID tenant"
    Write-Host "    (users, groups, and service principals), and optionally builds an"
    Write-Host "    interactive HTML dashboard summarizing eligible privileged access."
    Write-Host ""
    ...
    Write-Host "    -OutputPath       Folder to save the HTML dashboard (default: `$env:TEMP)"
    ...
    Write-Host '    Get-PIMEligibleEntraIDRoleAssignmentDetails -AccessToken $token -TenantName "Contoso" -GenerateHtmlDoc'
    Write-Host ""
    ...
    Write-Host '    Get-PIMEligibleEntraIDRoleAssignmentDetails -ClientId "<app-id>" -ClientSecret $secret -TenantId "<tenant-id>" -GenerateHtmlDoc'
    Write-Host ""
    Write-Host "  For full parameter and function documentation, run:" -ForegroundColor Green
    Write-Host "     Get-Help Get-PIMEligibleEntraIDRoleAssignmentDetails -Full"
    Write-Host ""
}

Function Get-PIMEligibleEntraIDRoleAssignmentDetails
{
...
"@
    ...
    $uri = "https://graph.microsoft.com/beta/roleManagement/directory/roleEligibilityScheduleInstances?`$expand=*&`$count=true"
    ...
    do
    {
        ...
        if (!$partialData)
        {
            Write-Host "PIM Role Eligible Assignments not found or do not exist" -ForegroundColor Red
        }
    ...
    $result = @($allLogs | ForEach-Object {
        ...
        [PSCustomObject]@{
            "Assignment State"             = if ($_.PSObject.Properties['assignmentType'] -and $_.assignmentType) { $_.assignmentType } else { "Eligible" }
    ...
    if ($GenerateHtmlDoc)
    {
        ...
        $eligibleCount  = ($result | Where-Object { $_.'Assignment State' -eq 'Eligible' } | Measure-Object).Count
        ...
        # Factors: % permanent eligibility, % high-priv permanent, % non-user principals
        ...
        function Build-TableRow {
            ...
            # For eligible assignments the state is always "Eligible"
            $state      = $item.'Assignment State'
            $stateClass = 'badge-eligible'
        ...
        # ── Pre-compute JS high-priv list ─────────────────────────────────────
        $highPrivRolesJs = ($highPrivRoles | ForEach-Object { "'$_'" }) -join ','

        # ── Pre-compute horizontal bar rows ──────────────────────────────────
...
<title>PIM Entra ID — $TenantName — Role Eligible Assignment Report</title>
<style>
/* ═══════════════════════════════════════════════════
...
.logo-icon{width:38px;height:38px;background:linear-gradient(135deg,var(--accent2),var(--accent3));border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:20px;margin-bottom:9px}
...
.tenant-dot{display:inline-block;width:7px;height:7px;background:var(--accent2);border-radius:50%;margin-right:5px;animation:blink 2.4s ease-in-out infinite}
...
.nav-btn.active{color:var(--accent2);background:rgba(57,197,207,.1)}
.nav-btn.active::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--accent2);border-radius:0 2px 2px 0}
...
body.light-theme .toggle-pill{background:var(--accent2)}
...
.btn:hover{border-color:var(--accent2);color:var(--accent2);background:rgba(57,197,207,.08)}
...
.stat-card:hover{transform:translateY(-2px);border-color:var(--accent2);box-shadow:0 2px 14px rgba(57,197,207,.12)}
...
.search-box:focus{border-color:var(--accent2);width:270px}
...
.badge-eligible{background:rgba(57,197,207,.12);color:var(--accent2);border:1px solid rgba(57,197,207,.30)}
...
.risk-alert-bar.info{background:rgba(57,197,207,.07);border-color:rgba(57,197,207,.25)}
...
<nav id="sidebar">
  <div class="sidebar-logo">
    ...
    <h1>PIM Eligible Assignments</h1>
...
<section class="page active" id="page-overview">
  <div class="page-header">
    <div>
      <div class="page-title">PIM Eligible Assignments — Overview</div>
  ...
  <div class="explainer-grid">
    <div class="explainer-card">
      ...
      <div class="explainer-title">What is an Eligible Assignment?</div>
      <div class="explainer-text">An <strong>eligible assignment</strong> means someone has been <em>pre-approved</em> to activate an admin role — but they do NOT have admin access right now. They must first request and activate the role through PIM before it becomes active.</div>
    </div>
    <div class="explainer-card">
      <div class="explainer-icon">🔄</div>
      <div class="explainer-title">Eligible vs Active — What's the Difference?</div>
      <div class="explainer-text"><strong>Active</strong> assignments grant admin rights immediately. <strong>Eligible</strong> assignments require the user to explicitly activate their role (and optionally provide justification or go through approval). This is the recommended PIM approach — access on demand, not always-on.</div>
    </div>
    <div class="explainer-card">
      <div class="explainer-icon">♾️</div>
      <div class="explainer-title">Permanent vs Time-Bound Eligibility</div>
      <div class="explainer-text">A <strong>permanent</strong> eligible assignment means a user can activate this role at any time indefinitely. A <strong>time-bound</strong> eligible assignment has an expiry — after which the user can no longer activate the role unless re-assigned.</div>
    </div>
    <div class="explainer-card">
      <div class="explainer-icon">⚠️</div>
      <div class="explainer-title">Why Review Eligible Assignments?</div>
      <div class="explainer-text">Even though eligible access isn't active right now, <strong>it still represents potential access</strong>. If an account is compromised, an attacker could activate a privileged role. Regular reviews ensure only the right people remain eligible.</div>
    </div>
  </div>
  ...
  <div class="stats-grid">
    <div class="stat-card c-cyan"><div class="stat-icon">⭐</div><div class="stat-value">$totalRows</div><div class="stat-label">Total Eligible</div><div class="stat-sub">Pre-approved role assignments</div></div>
    ...
    <div class="stat-card c-blue"><div class="stat-icon">👤</div><div class="stat-value">$totalUsers</div><div class="stat-label">People</div><div class="stat-sub">Individuals eligible for admin</div></div>
    <div class="stat-card c-purple"><div class="stat-icon">👥</div><div class="stat-value">$totalGroups</div><div class="stat-label">Groups</div><div class="stat-sub">Groups eligible for admin roles</div></div>
    <div class="stat-card c-amber"><div class="stat-icon">⚙️</div><div class="stat-value">$totalSPs</div><div class="stat-label">Apps &amp; Bots</div><div class="stat-sub">Service principals with eligibility</div></div>
    <div class="stat-card c-red"><div class="stat-icon">♾️</div><div class="stat-value">$permanentCount</div><div class="stat-label">Permanent Eligibility</div><div class="stat-sub">No expiry on the eligibility</div></div>
    <div class="stat-card c-green"><div class="stat-icon">⏱️</div><div class="stat-value">$timeBoundCount</div><div class="stat-label">Time-Bound Eligibility</div><div class="stat-sub">Eligibility with an end date</div></div>
    <div class="stat-card c-red"><div class="stat-icon">🚨</div><div class="stat-value">$highPrivCount</div><div class="stat-label">High-Privilege Eligible</div><div class="stat-sub">Most powerful roles pre-approved</div></div>
  </div>
  ...
  <div class="health-card">
    ...
    <div class="health-info">
      ...
      <p>Reflects how well your tenant manages <strong>eligible role exposure</strong> — keeping permanent eligibility low, limiting high-privilege pre-approvals, and minimising non-personal account eligibility. A higher score means lower risk.</p>
      <div class="health-bar-row">
        <span style="color:var(--red);font-size:12px;width:180px;flex-shrink:0">♾️ Permanent eligibility</span>
  ...
  <div class="chart-grid">
    <div class="panel">
      <div class="section-title">📊 Top Eligible Roles by Assignment Count</div>
      <div id="roleBarsContainer">$roleBarsHtml</div>
    </div>
    <div class="panel">
      <div class="section-title">🍩 Who Is Eligible for Admin Access?</div>
...
<section class="page" id="page-all">
  <div class="page-header">
    <div><div class="page-title">All Eligible Role Assignments</div><div class="page-subtitle">Every pre-approved admin role currently eligible in your tenant. Click column headers to sort.</div></div>
  </div>
  <div class="table-card">
    <div class="table-toolbar">
      <div><div class="toolbar-info-title">All Eligible Assignments</div><div class="toolbar-info-sub">$totalRows total entries</div></div>
    ...
    <div class="table-scroll">
      <table id="tbl-all">
        <thead><tr>
          ...
          <th onclick="sortTbl('all',2,this)">Eligible Role</th>
          <th onclick="sortTbl('all',3,this)">Person / Group / App</th>
          <th onclick="sortTbl('all',4,this)">Member Type</th>
          <th onclick="sortTbl('all',5,this)">Eligible Since (UTC)</th>
          <th onclick="sortTbl('all',6,this)">Eligibility Expires</th>
        </tr></thead>
...
<section class="page" id="page-users">
  <div class="page-header">
    <div><div class="page-title">👤 People Eligible for Admin Access</div><div class="page-subtitle">Individual user accounts pre-approved for admin roles. These users can self-activate their role through PIM.</div></div>
  </div>
  <div class="table-card">
    <div class="table-toolbar">
      <div><div class="toolbar-info-title">User Eligible Assignments</div><div class="toolbar-info-sub">$totalUsers entries</div></div>
      <div class="toolbar-right">
    ...
    <div class="table-scroll">
      <table id="tbl-users">
        <thead><tr>
          ...
          <th onclick="sortTbl('users',2,this)">Eligible Role</th>
          <th onclick="sortTbl('users',3,this)">Person / Group / App</th>
          <th onclick="sortTbl('users',4,this)">Member Type</th>
          <th onclick="sortTbl('users',5,this)">Eligible Since (UTC)</th>
          <th onclick="sortTbl('users',6,this)">Eligibility Expires</th>
        </tr></thead>
...
<section class="page" id="page-groups">
  <div class="page-header">
    <div><div class="page-title">👥 Groups Eligible for Admin Access</div><div class="page-subtitle">Security groups pre-approved for admin roles. Every member of these groups can activate the role.</div></div>
  </div>
  <div class="table-card">
    <div class="table-toolbar">
      <div><div class="toolbar-info-title">Group Eligible Assignments</div><div class="toolbar-info-sub">$totalGroups entries</div></div>
      <div class="toolbar-right">
    ...
    <div class="table-scroll">
      <table id="tbl-groups">
        <thead><tr>
          ...
          <th onclick="sortTbl('groups',2,this)">Eligible Role</th>
          <th onclick="sortTbl('groups',3,this)">Person / Group / App</th>
          <th onclick="sortTbl('groups',4,this)">Member Type</th>
          <th onclick="sortTbl('groups',5,this)">Eligible Since (UTC)</th>
          <th onclick="sortTbl('groups',6,this)">Eligibility Expires</th>
        </tr></thead>
...
<section class="page" id="page-sps">
  <div class="page-header">
    <div><div class="page-title">⚙️ Apps &amp; Bots Eligible for Admin Access</div><div class="page-subtitle">Automated applications and service accounts pre-approved for admin roles. Review carefully.</div></div>
  </div>
  <div class="table-card">
    <div class="table-toolbar">
      <div><div class="toolbar-info-title">App / Service Principal Eligible Assignments</div><div class="toolbar-info-sub">$totalSPs entries</div></div>
      <div class="toolbar-right">
    ...
    <div class="table-scroll">
      <table id="tbl-sps">
        <thead><tr>
          ...
          <th onclick="sortTbl('sps',2,this)">Eligible Role</th>
          <th onclick="sortTbl('sps',3,this)">Person / Group / App</th>
          <th onclick="sortTbl('sps',4,this)">Member Type</th>
          <th onclick="sortTbl('sps',5,this)">Eligible Since (UTC)</th>
          <th onclick="sortTbl('sps',6,this)">Eligibility Expires</th>
        </tr></thead>
...
<section class="page" id="page-risk">
  <div class="page-header">
    <div><div class="page-title">🔴 Risk Insights</div><div class="page-subtitle">Eligible assignments that pose the greatest security risk if activated by a compromised account.</div></div>
  </div>
  ...
  <div class="risk-alert-bar danger">
    ...
    <div style="flex:1">
      <div class="risk-alert-title">High-Privilege Roles with Permanent Eligibility — Highest Risk</div>
      <div class="risk-alert-desc">The most powerful admin roles (e.g. Global Administrator, Security Administrator) where eligibility <strong>never expires</strong>. If one of these accounts is compromised, an attacker can activate Global Admin at any time. Best practice is to set expiry dates on all high-privilege eligibility.</div>
    </div>
  ...
  <div class="risk-alert-bar warning">
    ...
    <div style="flex:1">
      <div class="risk-alert-title">All High-Privilege Eligible Assignments</div>
      <div class="risk-alert-desc">Total eligible assignments to highly sensitive roles regardless of expiry. Roles include: Global Administrator, Security Administrator, Exchange Administrator, Privileged Role Administrator, and others. Each represents a potential attack path if the account is compromised and the role is activated.</div>
    </div>
  ...
  <div class="risk-alert-bar info">
    <div class="risk-alert-icon">♾️</div>
    <div style="flex:1">
      <div class="risk-alert-title">All Permanent Eligible Assignments (Any Role)</div>
      <div class="risk-alert-desc">These eligibility assignments across all roles have no expiry. Microsoft recommends setting time-bound expiry on all PIM eligibility assignments to enforce periodic re-approval and reduce stale access risk.</div>
    </div>
    <div class="risk-alert-count" style="color:var(--accent2)">$permanentCount</div>
  </div>

  <!-- High-priv permanent table -->
  <div class="section-label" style="margin-top:22px">🔥 High-Privilege + Permanent Eligibility — Details</div>
  <div class="table-card">
    <div class="table-toolbar">
      <div><div class="toolbar-info-title">Critical: High-Privilege Roles with Permanent Eligibility</div><div class="toolbar-info-sub">$highPrivPerm entries — highest risk, review and set expiry immediately.</div></div>
      <div class="toolbar-right">
    ...
    <div class="table-scroll">
      <table id="tbl-riskperm">
        <thead><tr>
          ...
          <th onclick="sortTbl('riskperm',2,this)">Eligible Role</th>
          <th onclick="sortTbl('riskperm',3,this)">Person / Group / App</th>
          <th onclick="sortTbl('riskperm',4,this)">Member Type</th>
          <th onclick="sortTbl('riskperm',5,this)">Eligible Since (UTC)</th>
          <th onclick="sortTbl('riskperm',6,this)">Eligibility Expires</th>
        </tr></thead>
        <tbody id="tbody-riskperm">$(if($highPrivPermRows){''+$highPrivPermRows}else{'<tr><td colspan="7"><div class="empty-state"><div class="empty-icon">✅</div><p>No high-privilege permanent eligible assignments found — great work!</p></div></td></tr>'})</tbody>
      </table>
    </div>
  ...
  <!-- All high-priv table -->
  <div class="section-label">⚠️ All High-Privilege Eligible Assignments</div>
  <div class="table-card">
    <div class="table-toolbar">
      <div><div class="toolbar-info-title">All High-Privilege Eligible Assignments</div><div class="toolbar-info-sub">Includes both permanent and time-bound — $highPrivTotal total entries.</div></div>
      <div class="toolbar-right">
    ...
    <div class="table-scroll">
      <table id="tbl-riskhigh">
        <thead><tr>
          ...
          <th onclick="sortTbl('riskhigh',2,this)">Eligible Role</th>
          <th onclick="sortTbl('riskhigh',3,this)">Person / Group / App</th>
          <th onclick="sortTbl('riskhigh',4,this)">Member Type</th>
          <th onclick="sortTbl('riskhigh',5,this)">Eligible Since (UTC)</th>
          <th onclick="sortTbl('riskhigh',6,this)">Eligibility Expires</th>
        </tr></thead>
        <tbody id="tbody-riskhigh">$(if($highPrivRows){''+$highPrivRows}else{'<tr><td colspan="7"><div class="empty-state"><div class="empty-icon">✅</div><p>No high-privilege eligible assignments found.</p></div></td></tr>'})</tbody>
      </table>
    </div>
...
<section class="page" id="page-export">
  <div class="page-header">
    <div><div class="page-title">⬇️ Export Data</div><div class="page-subtitle">Download eligible role assignment data in your preferred format.</div></div>
  </div>
  ...
  <div class="export-grid">
    <div class="export-card">
      ...
      <div class="export-card-desc">Downloads all $totalRows eligible assignments as a <strong>comma-separated values (.csv)</strong> file. Ideal for opening in <strong>Microsoft Excel</strong> or importing into other tools.</div>
    ...
    <div class="export-card">
      ...
      <div class="export-card-desc">Downloads all $totalRows assignments as a <strong>structured JSON (.json)</strong> file. Best for developers, automation scripts, or ingesting into SIEM tools like Microsoft Sentinel.</div>
    ...
    <div class="export-card">
      ...
      <div class="export-card-desc">Downloads only the <strong>$highPrivCount high-privilege eligible assignments</strong> as a CSV. Useful for sharing with your security team or management for immediate review.</div>
      <button class="btn" onclick="exportHighPrivCSV()">⬇ Download Risk CSV</button>
    </div>
    <div class="export-card">
      <div class="export-card-icon">🖨️</div>
      <div class="export-card-title">Print / Save as PDF</div>
      <div class="export-card-desc">Use your browser's built-in print function to <strong>save this dashboard as a PDF</strong>. Great for audit records, management reports, or offline sharing.</div>
  ...
  <div class="panel">
    <table style="width:auto;font-size:13px;border-collapse:collapse">
      ...
      <tr><td style="padding:7px 24px 7px 0;color:var(--muted);font-weight:600">Total Eligible Assignments</td><td style="padding:7px 0;color:var(--text);font-weight:700">$totalRows</td></tr>
      ...
      <tr><td style="padding:7px 24px 7px 0;color:var(--muted);font-weight:600">Data Source</td><td style="padding:7px 0;color:var(--text)">Microsoft Graph API (Beta) — roleEligibilityScheduleInstances</td></tr>
    </table>
  </div>
</section>
...
function exportCSV(){
  dlFile(CSV_DATA,'PIM_EligibleAssignments_${TenantName}_$todayDateShort.csv','text/csv');
  showToast('✅ Exported $totalRows eligible assignments as CSV');
}
function exportJSON(){
  dlFile(JSON_DATA,'PIM_EligibleAssignments_${TenantName}_$todayDateShort.json','application/json');
  showToast('✅ Exported $totalRows eligible assignments as JSON');
}
function exportHighPrivCSV(){
  ...
  dlFile([header,...filtered].join('\n'),'PIM_EligibleHighPriv_${TenantName}_$todayDateShort.csv','text/csv');
  showToast('✅ Exported high-privilege eligible assignments as CSV');
}
...
"@
        ...
        $htmlFile = Join-Path -Path $OutputPath -ChildPath "PIMEntraIDRoleEligibleAssignment_$($TenantName -replace '\s','_')_$(Get-Date -Format 'yyyyMMdd_HHmmss').html"
    ...
    $stats1 = $result | Group-Object "Role Name" | ForEach-Object {
        $roleName      = $_.Name
        $eligibleCount = ($_.Group | Where-Object { $_.'Assignment State' -eq 'Eligible' } | Measure-Object).Count
