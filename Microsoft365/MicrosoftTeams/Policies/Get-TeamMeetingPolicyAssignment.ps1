...
Function Get-TeamMeetingPolicyAssignment {
    [CmdletBinding()]
    param (
    ...
    Begin {
        ...
        $PolicyTypeFilter = 'TeamsMeetingPolicy'
        ...
        Write-Host ""
        Write-Host "  🔍  Starting Teams Meeting Policy Assignment report..." -ForegroundColor Cyan
    }

    Process {
        # Accumulate UPNs supplied via pipeline
    ...
    End {
        ...
        if ($pipelineUpns.Count -gt 0) {
            ...
            # Resolve all Teams-licensed users via paginated /users call
        ...
        foreach ($user in $resolvedUsers) {
            $totalProcessed++
            Write-Progress -Activity "Retrieving meeting policy assignments" `
        ...
        Write-Progress -Activity "Retrieving meeting policy assignments" -Completed
        ...
        if ($ExportFormat -eq "CSV" -and $ExportPath) {
            Try {
                ...
                Write-Host "  📄  Meeting policy report exported → $ExportPath" -ForegroundColor Green
            }
            Catch {
