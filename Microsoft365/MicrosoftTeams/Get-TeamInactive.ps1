        ...
        foreach ($id in $TeamId)
        {
            Try
            {
                ...
                if ($createdRaw)
                {
                    ...
                    {
                        $createdDate = $null
                        Try { $createdDate = [DateTime]$createdRaw } Catch { $createdDate = $null }
                        if ($createdDate) { $ageInDays = ((Get-Date) - $createdDate).Days }
                    }
                }
                ...
                if ($activityMap.ContainsKey($id))
                {
                    ...
                    {
                        $parsed = $null
                        Try { $parsed = [DateTime]$lastActivityRaw } Catch { $parsed = $null }
                        if ($parsed)
                        {
