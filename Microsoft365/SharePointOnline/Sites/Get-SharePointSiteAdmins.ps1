...
Function Get-SharePointSiteAdmins
{
    ...
    Write-Progress -Activity "Get-SharePointSiteAdmins" -Status "Resolving site scope..." -PercentComplete 0
    try
    {
    ...
    {
        $siteCounter++
        Write-Progress -Activity "Get-SharePointSiteAdmins" -Status "Processing $($site.Url)" `
    ...
    Write-Progress -Activity "Get-SharePointSiteAdmins" -Completed
    Write-Verbose "Built report with $($report.Count) admin row(s) across $($targetSites.Count) site(s)."
    ...
    if ($OutputPath)
    {
        ...
        else
        {
            ...
            $outFile   = Join-Path -Path $OutputPath -ChildPath "SharePointSiteAdmins_$timestamp.csv"
        }
        try
        {
