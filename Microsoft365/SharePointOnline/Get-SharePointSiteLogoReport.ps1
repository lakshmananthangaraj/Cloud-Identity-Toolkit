...
Function Get-SharePointSiteLogoReport
{
    ...
    Write-Progress -Activity "Get-SharePointSiteLogoReport" -Status "Resolving site scope..." -PercentComplete 0
    try
    {
    ...
    {
        $siteCounter++
        Write-Progress -Activity "Get-SharePointSiteLogoReport" -Status "Processing $($site.Url)" `
            -PercentComplete (($siteCounter / [Math]::Max($totalSites,1)) * 100)

        try
        {
            $siteConn = Connect-ToSPO -Url $site.Url
            # $web      = Get-PnPWeb -Includes SiteLogoUrl,ThemedSiteLogoUrl,WebTemplate,Title -Connection $siteConn -ErrorAction Stop
            $web      = Get-PnPWeb -Includes SiteLogoUrl,WebTemplate,Title -Connection $siteConn -ErrorAction Stop

            $hasCustomLogo = -not [string]::IsNullOrWhiteSpace($web.SiteLogoUrl) -or -not [string]::IsNullOrWhiteSpace($web.ThemedSiteLogoUrl)

            if ($MissingLogoOnly -and $hasCustomLogo) { continue }
            ...
            $report.Add([PSCustomObject]@{
                ...
                ThemedSiteLogoUrl = $web.ThemedSiteLogoUrl
    ...
    Write-Progress -Activity "Get-SharePointSiteLogoReport" -Completed
    ...
    if ($OutputPath)
    {
        ...
        else
        {
            ...
            $outFile   = Join-Path -Path $OutputPath -ChildPath "SharePointSiteLogoReport_$timestamp.csv"
        }
        try
        {
