...
Function Get-SharePointSiteOrphanedUsers
{
    ...
    Write-Progress -Activity "Get-SharePointSiteOrphanedUsers" -Status "Resolving site scope..." -PercentComplete 0
    try
    {
    ...
    {
        $siteCounter++
        Write-Progress -Activity "Get-SharePointSiteOrphanedUsers" -Status "Processing $($site.Url)" `
            -PercentComplete (($siteCounter / [Math]::Max($totalSites,1)) * 100)

        try
        {
            $siteConn  = Connect-ToSPO -Url $site.Url
            $siteUsers = Get-PnPUser -Connection $siteConn -ErrorAction Stop | Where-Object {
                $_.PrincipalType -eq 'User' -and $_.LoginName -notmatch 'SHAREPOINT\\system' -and $_.Email
            }
            ...
            foreach ($u in $siteUsers)
            {
                ...
                try
                {
                    $null = Get-PnPAzureADUser -Identity $u.AadObjectId -Connection $siteConn -ErrorAction Stop
                }
                catch
                {
                    $isOrphaned = $true
                    $resolveErr = $_.Exception.Message
                }
    ...
    Write-Progress -Activity "Get-SharePointSiteOrphanedUsers" -Completed
    Write-Verbose "Built report with $($report.Count) user row(s) across $($targetSites.Count) site(s)."

    if ($report.Count -eq 0)
    {
        Write-Host "No users were found matching the specified criteria." -ForegroundColor Yellow
    }
    #endregion
    ...
    if ($OutputPath)
    {
        ...
        else
        {
            ...
            $outFile   = Join-Path -Path $OutputPath -ChildPath "SharePointSiteOrphanedUsers_$timestamp.csv"
        }
        try
        {
